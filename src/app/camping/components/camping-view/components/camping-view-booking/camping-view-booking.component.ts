...
import { convertArrayToObject, daysBetweenDates, formatNumber, isEmptyObject } from '@utils/functions';
...
export class CampingViewBookingComponent {
  ...
  getTotalFee(id: string | null = null) {
    if (this.exitDate && this.entryDate) {
      if(id) {
        const fee = this.lodgingsToBook[id] * daysBetweenDates(this.exitDate, this.entryDate) * this.lodgings[id].feePerNight;
        return formatNumber(fee, { decimals: 2, currency: '€'});
      }
      const num =  Object.keys(this.lodgingsToBook).reduce((prev, curr) => {
        return prev + this.lodgingsToBook[curr] * daysBetweenDates(this.exitDate, this.entryDate) * this.lodgings[curr].feePerNight
      }, 0)
