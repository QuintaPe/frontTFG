...
export class CreateCampingComponent implements OnInit {
  ...
  breadcrumb = [
    { name: this.translate.instant('camping.campings'), route: CAMPINGS_MANAGEMENT_ROUTES.CAMPINGS.url },
  ...
  async handleSubmit() {
    if (this.page > 2) {
      ...
      try {
        ...
        this.router.navigateByUrl(CAMPINGS_MANAGEMENT_ROUTES.CAMPINGS.url);
      } catch (err) {
        throw err;
      }
    } else {
